...
public class TestUtil {
    ...
    public static final String certWithMultipleAttributes = "MIIChzCCAi6gAwIBAgIURilAHeqwLu/fNUv8eZoGPRh3H4IwCgYIKoZIzj0EAwIw" +
     ...
     */
    public static String createCertWithIdentityAttributes(String attributeValue) throws Exception {

        // Use existing certificate with attributes
        byte [] decodedCert = Base64.getDecoder().decode(certWithMultipleAttributes);
        // Create a certificate holder and builder
        X509CertificateHolder certHolder = new X509CertificateHolder(decodedCert);
        X509v3CertificateBuilder certBuilder = new X509v3CertificateBuilder(certHolder);

        // special OID used by Fabric to save attributes in x.509 certificates
        String FABRIC_CERT_ATTR_OID = "1.2.3.4.5.6.7.8.1";
        // Write the new attribute value
        byte[] extDataToWrite = attributeValue.getBytes();
        certBuilder.replaceExtension(new ASN1ObjectIdentifier(FABRIC_CERT_ATTR_OID), true, extDataToWrite);
        ...
        X509CertificateHolder builtCert = certBuilder.build(contentSigner);
        X509Certificate certificate = (X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(builtCert.getEncoded()));
